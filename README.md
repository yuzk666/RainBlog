...
RainBlog 是一个以中文长文阅读为核心的个人博客。它适合写日记、随笔和生活感想，公开文章可以分享给朋友，草稿与私密文章只对管理员可见，注册读者可以在公开文章下发表评论。

项目刻意保持简单：Django 模板负责页面，SQLite 保存数据，Django Admin 负责写作与发布；用户系统沿用 Django 内置认证，不包含前后端分离或不必要的服务层。
...
- 普通用户账号、密码与公开昵称注册，登录后可修改昵称和已知密码
- 自定义登录与写作台；管理员可管理文章，普通用户不能进入写作台或 Django Admin
- 登录用户评论即时公开，身份由服务端绑定，同时保留 HTML 转义、蜜罐反垃圾与 30 秒提交冷却
- 写作台评论管理，可处理历史状态并永久删除不合适的评论
...
- 个人设置：http://127.0.0.1:8000/profile/
- 修改密码：http://127.0.0.1:8000/password/change/
...
- 注册只收集唯一账号、密码和公开昵称，不要求邮箱；账号会统一为小写，登录比较不区分大小写。
- 密码始终通过 Django 的密码哈希与强度验证处理，不会以明文保存。
- 匿名访客可以阅读评论；登录用户只填写评论内容，作者身份和昵称由服务端确定，提交后立即公开。
- 评论会保存发表时的昵称快照。以后修改昵称只影响新评论，不会重写旧评论。
- 评论内容按纯文本展示，HTML 会被转义；隐藏蜜罐和同一会话 30 秒冷却可减少简单刷屏。
- 草稿、私密文章和尚未到发布时间的文章不接受普通用户评论。
- `/dashboard/comments/` 保留历史待审核、已拒绝状态的管理能力；管理员也可删除明显不合适的评论。
- 本轮不收集邮箱，也不提供自助找回密码。忘记密码的用户需要联系管理员，管理员在项目目录执行 `python manage.py changepassword 账号` 后按提示设置新密码。
...
测试覆盖首页、文章权限、分类、标签、归档、404、Markdown 安全、注册与密码哈希、大小写重复账号、安全跳转、昵称与密码修改、管理员兼容、写作台权限、文章增删改查、登录评论、身份防伪、HTML 转义、蜜罐和提交冷却。
...
2. 打开 `/register/` 注册一个普通账号，确认可以登录、修改昵称和密码，并且无法进入 `/dashboard/` 与 `/admin/`。
3. 打开 `/login/` 和 `/admin/`，确认原管理员可以登录、写文章、管理评论和上传封面。
4. 在 `.env` 再次确认 `DJANGO_DEBUG=False`。
5. 访问一个不存在的地址，确认显示自定义 404 而不是 Django 调试页。
6. 公开、私密、草稿各建一篇，退出管理员账号后确认私密与草稿都返回 404；普通用户登录后只可评论公开文章，且评论立即显示。
7. 确认 HTTPS 有效；`pythonanywhere.com` 子域名由平台提供 HTTPS 证书。
8. 若出现错误，优先查看 Web 页面的 Error log 和 Server log。
...
这些功能都可以在保持当前结构的前提下逐步增加，目前没有为了它们引入额外服务。
